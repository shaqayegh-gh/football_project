...
class Robot(Field):
    ...
    def move_robot(self):
        if self.charge>0:
            ...
            while ((self.pos[0] + self.direction[0], self.pos[1] + self.direction[1]) in self.full_pos) or\
                    ...
                    (self.pos[1] + self.direction[1]) < 15 or (self.pos[1] + self.direction[1]) > (20.5 * 30):     # check if the selected pos is empty and not going out the field
    ...
    def move_robot2first(self,x,y):
        self.player.move(x-self.pos[0],y-self.pos[1])  # move the player into the new pos based on direction
        self.new_pos = (self.player.getCenter().x, self.player.getCenter().y)
        self.full_pos.remove(self.pos)
        self.full_pos.append(self.new_pos)
        self.pos = self.new_pos
...
class Halfback1(Robot):  # the first halfback of the  team
    ...
    def manage(self,x,y):  # set the first manage of the players in the field
        self.player = Circle(Point(x,y), 10)
    ...
    def shoot(self):
        if self.color=="red":       #the red team should shoot to the right
            self.shoot_power=(random.choice([30,60,90,120,150,180]),0)
